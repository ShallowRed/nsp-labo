...
3. Dans InDesign, panneau Scripts, dossier « Passe couleur NSP » (`exports/spectre/bascule-rapport/`) :
   ...
   - `composer-graphiques.jsx` : variante qui cale aussi le titre (bord gauche sur l'axe) et les précisions (toute la largeur du fond). Non retenue pour la V9, gardée pour référence.
   - `numeroter-graphiques.jsx` : pose sur chaque graphique G1 à G91 un numéro automatique (liste « Graphiques », style « Numéro graphique », Poppins SemiBold 7 pt pétrole) dans un petit bloc en haut à droite de la zone du graphique, au-dessus du titre, calé à 24 pt du bord droit du fond. Ordre de lecture : page, puis haut vers bas. Relançable.
   - `corriger-synthese.jsx` : remplacements de texte de la synthèse issus du Word du 15 septembre 2026, à usage unique.
